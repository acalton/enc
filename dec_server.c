...
* 2. Upon connection, read an encrypted message and key from the client.
* 3. Use the key to decode the message,
* 4. Send decoded message back to client.
*/
...
// Decode a given encrypted message using the key used in encryption
char* decryptMessage(char* encMessage, char* key) {
    // convert encrypted message to array of num
    int intEncBuffer[strlen(encMessage)];
    int i = 0;
    while (i < strlen(encMessage)) {
        intEncBuffer[i] = convertChar(encMessage[i]);
        i++;
    }
    ...
    // subtract encrypted message nums from key nums (adjust negative values)
    int intDecBuffer[strlen(encMessage)];
    int subNums;
    i = 0;
    while (i < strlen(encMessage)) {
        subNums = intEncBuffer[i] - intKeyBuffer[i];
        if (subNums < 0) {  // add 27 to keep in range of allowed char
            subNums += 27;
        }
        intDecBuffer[i] = subNums;
        i++;
    }

    // convert subtracted nums to chars
    char* decMessage = malloc(strlen(encMessage));
    i = 0;
    while (i < strlen(encMessage)) {
        decMessage[i] = convertInt(intDecBuffer[i]);
        i++;
    }
...
int main(int argc, char* argv[]) {
    ...
    while (1) {
        ...
        case 0: {
            ...
            char* decryptedMessage = decryptMessage(message, key);
            // Send decrypted message back to the client
            int totalCharsWritten = 0;
            int charsWritten;
            while (totalCharsWritten < strlen(decryptedMessage)) {
                charsWritten = send(connectionSocket, decryptedMessage, strlen(decryptedMessage), 0);
                if (charsWritten < 0) {
                    error("SERVER: ERROR sending decrypted message to client");
                    break;
                }
