...
* 2. Open message and key files and read them into buffers.
* 3. Send message and key to server.
* 4. Receive encrypted message from client and print it to stdout.
*/
...
int main(int argc, char* argv[]) {
    ...
    char encMessage[strlen(message)];
    int totalCharsRead = 0;
    while (totalCharsRead < strlen(message)) {
        charsRead = recv(socketFD, encMessage, sizeof(encMessage), 0);
        if (charsRead < 0) {
            error("CLIENT: ERROR receiving encrypted message from server");
            break;
        }
